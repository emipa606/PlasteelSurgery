...
public class PS_Recipes_Surgery_Laryngoplasty : Recipe_Surgery
{
    public override IEnumerable<BodyPartRecord> GetPartsToApplyOn(Pawn pawn, RecipeDef recipe)
    {
        ...
        var hasHediff = pawn?.health?.hediffSet.hediffs
        ...
        Bill bill)
    {
        ...
        if (!CheckSurgeryFail(billDoer, pawn, ingredients, part, bill))
        {
            ...
            var hediff =
                (pawn?.health?.hediffSet?.hediffs).FirstOrDefault(x =>
                    x.def == PS_DefOf.PS_HediffDefs_BotchedLaryngoplasty);
            if (hediff != null)
            {
        ...
        {
            var hasHediff =
                (pawn?.health?.hediffSet?.hediffs).Any(x => x.def == PS_DefOf.PS_HediffDefs_BotchedLaryngoplasty);
            if (!hasHediff)
            {
